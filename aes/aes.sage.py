...
class AES(object):
    ...
    @staticmethod
    def SubWord(word):
        word = AES.RotWordLeft(word,_sage_const_1 )
    ...
    def AddRoundKey(self,number):
        ...
        self.roundkey = matrix(SR, _sage_const_4 , _sage_const_4 , [self.keystorage[i] for i in range(counter,counter+_sage_const_4 )])
        #print "self.roundkey:\n%s" % self.roundkey
        tmp = []
    ...
    def ShiftRow(self):
        ...
        for x in ((self.state[i],i) for i in range(self.Nb)):
            tmp.append(AES.RotWordLeft(list(x[_sage_const_0 ]),x[_sage_const_1 ]))
        self.state = matrix(SR,self.Nb,_sage_const_4 ,tmp)
        #print "state after:\n%s" % self.state

    def MixColumns(self):
        R = PolynomialRing(GF(_sage_const_2 ), 'x', names=('x',)); (x,) = R._first_ngens(1)
        S = QuotientRing(R, R.ideal(x**_sage_const_8 +x**_sage_const_4 +x**_sage_const_3 +x+_sage_const_1 ), names=('y',)); (y,) = S._first_ngens(1)
        print S
